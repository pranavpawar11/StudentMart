...
try {
    $query = "
        ...
        WHERE 
            o.buyer_id = :user_id AND r.status = 'completed'
        ORDER BY 
            r.request_date DESC;
    ";
    ...
    if ($results) {
        ...
        foreach ($results as $request) {
            ...
            echo '<p class="card-text"><strong>Request Status:</strong> ' . htmlspecialchars($request['request_status']) . '</p>';
            echo '<p class="card-text"><strong>Total Price:</strong> ₹ ' . htmlspecialchars($request['total_price']) . '</p>';
            ...
            echo '<p><strong>Seller Phone:</strong> ' . htmlspecialchars($request['seller_phone']) . '</p>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
            echo '<div class="modal-footer">';
            echo '<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>';
            echo '<a target="_blank" href="https://api.whatsapp.com/send?phone=' . htmlspecialchars($request['seller_phone']) . '" class="btn btn-success">Chat with Seller</a>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
