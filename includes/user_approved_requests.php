...
try {
    ...
    if ($results) {
        ...
        foreach ($results as $order) {
            ...
            echo ($order['tracking_status'] === 'placed') ? 'bg-secondary' : '';
            echo ($order['tracking_status'] === 'shipped') ? 'bg-info' : '';
            echo ($order['tracking_status'] === 'out_for_delivery') ? 'bg-warning' : '';
            echo ($order['tracking_status'] === 'delivered') ? 'bg-success' : '';
            echo '" role="progressbar" style="width: ';
            echo ($order['tracking_status'] === 'placed') ? '25%' : '';
            echo ($order['tracking_status'] === 'shipped') ? '50%' : '';
            echo ($order['tracking_status'] === 'out_for_delivery') ? '75%' : '';
            echo ($order['tracking_status'] === 'delivered') ? '100%' : '';
            echo '">' . ucwords(str_replace('_', ' ', $order['tracking_status'])) . '</div>';
            echo '</div>';
            echo '</div>';
