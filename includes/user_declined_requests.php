...
try {
    $query = "
        ...
        WHERE 
            o.seller_id = :user_id AND r.status = 'declined'
        ORDER BY 
            r.request_date DESC;
    ";
    ...
    if ($results) {
        ...
        echo '<h2 class="text-center mb-4">Declined Requests</h2>';
        echo '<div class="row">';
        ...
        foreach ($results as $request) {
            ...
            echo '<button class="btn btn-primary btn-block mt-auto view-details" data-toggle="modal" data-target="#detailsModal_' . $request['request_id'] . '">View Details</button>';
            echo '<button class="btn btn-success btn-block mt-2" onclick="approveRequest(' . $request['request_id'] . ')">Approve Request</button>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
        ...
        echo '<div class="alert alert-info text-center">No declined requests found</div>';
    }
