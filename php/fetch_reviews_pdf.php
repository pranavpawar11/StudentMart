...
try {
    ...
    $sql = "SELECT pdf_reviews.*, user.fname, user.lname, user.photo FROM pdf_reviews 
            JOIN user ON pdf_reviews.user_id = user.user_id 
            WHERE pdf_id = ? 
    ...
    if (empty($reviews)) {
        ...
        echo '<p class="no-reviews-message">No reviews yet for this PDf. Be the first to review!</p> <hr>';
    } else {
